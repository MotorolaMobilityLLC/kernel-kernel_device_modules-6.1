...
#include "mot_tele_dw9784_i2c.h"
#include "mot_tele_dw9784_ois.h"

#include "hf_manager.h"
#define FW_UPDATE_DEBUG
...
#define DW9784_NAME				"mot_tele_dw9784"
...
int GenerateFirmwareContexts(void)
{
	...
	i4RetValue = request_firmware(&dw9784fw, "mot_tele_dw9784.prog", NULL);
...
static int dw9784_init(struct dw9784_device *dw9784)
{
	...
	ois_reset_conflict();
	//ret = ois_i2c_wr(m_client, 0x7020, 0x0000);//master

	ret = ois_i2c_rd(client, 0x7011, &lock_ois);
	LOG_INF("Check HW lock_ois: %x\n", lock_ois);
	ret = ois_i2c_rd(client, 0x7194, &is_gyro);
...
static int dw9784_power_on(struct dw9784_device *dw9784)
{
	...
	if (ret < 0)
		return ret;
	LOG_INF("dw9784 power on success");
...
static int dw9784_probe(struct i2c_client *client)
{
	...
	hw_nums = ARRAY_SIZE(clk_names);
	LOG_INF("clk_names hw_nums:0x%d\n", hw_nums);
	if (hw_nums > CLK_MAXSIZE)
		hw_nums = CLK_MAXSIZE;
	for (i = 0; i < hw_nums; i++) {
		...
		if (IS_ERR(dw9784->clk[i])) {
			dw9784->clk[i] = NULL;
			LOG_INF("cannot get %s clk, i:%d\n", clk_names[i], i);
		}
	}

	hw_nums = ARRAY_SIZE(ldo_names);
	LOG_INF("ldo_names hw_nums:%d\n", hw_nums);
	if (hw_nums > REGULATOR_MAXSIZE)
		hw_nums = REGULATOR_MAXSIZE;
	for (i = 0; i < hw_nums; i++) {
		dw9784->ldo[i] = devm_regulator_get(dev, ldo_names[i]);
		if (IS_ERR(dw9784->ldo[i])) {
			LOG_INF("cannot get %s regulator, i:%d\n", ldo_names[i], i);
			dw9784->ldo[i] = NULL;
		}
	}

	hw_nums = ARRAY_SIZE(pio_names);
	LOG_INF("pio_names hw_nums:%d\n", hw_nums);
	if (hw_nums > PINCTRL_MAXSIZE)
		hw_nums = PINCTRL_MAXSIZE;
	...
	if (IS_ERR(dw9784->pio)) {
		...
		for (i = 0; i < hw_nums; i++) {
			...
			if (IS_ERR(dw9784->pio_state[i])) {
				...
				LOG_INF("cannot get %s pinctrl,i:%d\n", pio_names[i],i);
			}
		}
	}
